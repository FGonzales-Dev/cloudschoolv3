...
@php
    ...
    $textColor =
        empty($component->text_color_light) && empty($component->text_color_dark)
            ? 'text-color-14 dark:text-white'
            : 'text-[var(--text-color-light)] dark:text-[var(--text-color-dark)]';
    $buttonColor =
        empty($component->plan_btn_color_light) &&
        empty($component->plan_btn_color_dark) &&
        empty($component->plan_btn_text_color_light) &&
        empty($component->plan_btn_text_color_dark)
            ? 'text-white dark:text-color-14 bg-color-14 dark:bg-white'
            : 'text-[var(--btn-text-color-light-1)] dark:text-[var(--btn-text-color-dark-1)] bg-[var(--btn-color-light-1)] dark:bg-[var(--btn-color-dark-1)]';

    $planButtonColor =
        empty($component->btn_color_light) &&
        empty($component->btn_color_dark) &&
        empty($component->btn_text_color_light) &&
        empty($component->btn_text_color_dark)
            ? 'nav-parent active'
            : 'text-[var(--plan-btn-text-color-light)] dark:text-[var(--plan-btn-text-color-dark)] bg-[var(--plan-btn-color-light)] dark:bg-[var(--plan-btn-color-dark)]';

    $planTextColor =
        empty($component->plan_text_color_light) && empty($component->plan_text_color_dark)
            ? 'text-color-14 dark:text-white'
            : 'text-[var(--plan-text-color-light)] dark:text-[var(--plan-text-color-dark)]';
    $creditTextColor =
        empty($component->credit_text_color_light) && empty($component->credit_text_color_dark)
            ? 'text-color-14 dark:text-white'
            : 'text-[var(--credit-text-color-light)] dark:text-[var(--credit-text-color-dark)]';

    $creditTextColorHead =
        empty($component->credit_text_color_light_head) && empty($component->credit_text_color_dark_head)
            ? 'text-[#E22861] dark:text-[#FCCA19]'
            : 'text-[var(--credit-text-color-light-head)] dark:text-[var(--credit-text-color-dark-head)]';

    $creditButtonColor =
        empty($component->credit_btn_color_light) &&
        empty($component->credit_btn_color_dark) &&
        empty($component->credit_btn_text_color_light) &&
        empty($component->credit_btn_text_color_dark)
            ? 'text-white magic-bg'
            : 'text-[var(--btn-text-color-light-2)] dark:text-[var(--btn-text-color-dark-2)] bg-[var(--btn-color-light-2)] dark:bg-[var(--btn-color-dark-2)]';

    $bgColor =
        empty($component->main_bg_color_light) && empty($component->main_bg_color_dark)
            ? ''
            : 'bg-[var(--bg-color-light)] dark:bg-[var(--bg-color-dark)]';
@endphp
...
{{ $component->background_type == 'backgroundImage' ? 'plan-bg-' . $component->id : $bgColor }}"
    ...
    <p
        class="font-Figtree lg:w-[700px] mx-auto mt-3 px-5 md:px-10 xl:px-5 flex items-center justify-center font-normal text-center 6xl:text-18 text-16 {{ $textColor }}">
        {!! $component->body !!}
    </p>
    ...
    <div>
        ...
        <div class="tab-content mt-6 xl:mt-8" id="tabs-tabContent">
                ...
                <div style="height: auto;"
                    ...
                    @foreach ($packages as $key => $package)
                        @foreach ($package['billing_cycle'] as $billing_cycle => $value)
                            @continue($value == 0)
                            <div style={{ $package['parent_class'] }} plan-parent plan-{{ $billing_cycle }}
                                ...
                                <div style="height: 100%; border-radius: 8px;"
                                    ...
                                    @elseif ($subscription?->package?->id == $package['id'] && $billing_cycle == $subscription?->billing_cycle)
                                        @if ($subscription?->package?->renewable)
                                            <button type="submit"
                                                class="{{ $package['button'] . ' ' . $buttonColor }} plan-loader flex gap-3 w-full justify-center text-center border border-[#808080] rounded-[12px]">{{ __('Renew Plan') }}</button>
                                        @endif
                                    @elseif (preference('subscription_change_plan') &&
                                        ...
                                        <button type="submit"
                                            class="{{ $package['button'] . ' ' . $buttonColor }} plan-loader flex gap-3 w-full justify-center text-center border border-[#808080] rounded-[12px]">{{ __('Upgrade Plan') }}</button>
                                    @elseif (preference('subscription_change_plan') &&
            ...
            <div class="tab-pane fade" id="tabs-category-tab-id-1" role="tabpanel"
                ...
                <div
                    class="lg:mt-6 mt-[60px] 6xl:gap-10 lg:gap-5 gap-6 lg:px-0 md:px-10 px-5 w-full flex flex-wrap justify-center">
                    @foreach ($credits as $key => $credit)
                        <div
                            class="bg-small-package border border-color-DF bg-white dark:border-color-47 dark:bg-[#3A3A39] rounded-xl p-5 h-max lg:w-[30.33%] pricing-width w-full">
                            <div class="flex flex-col justify-between gap-5 h-full">
                                <div>
                                    <p
                                        class="{{ $creditTextColorHead }} text-[14px] font-medium leading-[22px] font-Figtree wrap-anywhere">
                                        {{ $credit->name }}</p>
                                    <p
                                        class="{{ $creditTextColor }} text-[28px] font-medium font-RedHat leading-10 mt-0.5 mb-5 flex items-baseline gap-1">
                                        <span
                                            class="text-[36px] font-bold leading-[44px] wrap-anywhere ">{{ formatNumber($credit->price) }}</span>
                                    </p>
                                ...
                                @if (preference('apply_coupon_onetime'))
                                    <form action="{{ route('user.subscription.checkout') }}" method="GET"
                                        class="button-need-disable">
                                    @else
                                        <form action="{{ route('user.credit.store') }}" method="POST"
                                            class="button-need-disable">
                                            @csrf
                                @endif
                                <input type="hidden" name="package_id" value="{{ $credit->id }}">
                                <input type="hidden" name="sending_url"
                                    value="{{ techEncrypt(route('user.credit.store')) }}">
                                <input type="hidden" name="billing_cycle" value="onetime">
                                <button type="submit"
                                    class="h-max w-max rounded-xl text-[14px] leading-[22px] font-semibold py-3 px-[25px] {{ $creditButtonColor }}">
                                    <span>{{ __('Buy Now') }}
                                    </span>
                                </button>
                                </form>
