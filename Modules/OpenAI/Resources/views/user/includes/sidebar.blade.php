...
@if ($wordLeft && auth()->user()->id == $userId)
    <div class="bg-white dark:bg-[#474746] p-3 rounded-xl flex items-center justify-start mt-6 gap-2.5 mx-6">
        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18" fill="none">
            ...
            <defs>
                <linearGradient id="paint0_linear_4514_3509" x1="10.5204" y1="15.7845" x2="2.32033" y2="5.3758"
...
<form id="openai-form">
    <div class="px-5 py-[22px] sm:py-8 xl:p-6 xl:pb-[56px] font-Figtree">
                    ...
                    <div class="tom-select-img">
                        <select id="use-case-image"
                            ...
                            @foreach ($useCases as $k => $data)
                                <option data-src="{{ $data->fileUrl() }}" value="{{ $data->slug }}"
                                    {{ $data->slug == $slug ? 'selected' : '' }}>
                                    {{ $data->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                @foreach ($options as $key => $value)
                    @if ($value->type == 'text')
                        <div class="relative flex flex-col mt-6">
                            ...
                            <input
                                ...
                                data-input-name="{{ $value->key }}" maxlength="{{ preference('short_desc_length') }}"
                                name="{{ $value->key }}">
                            <p
                    ...
                    @elseif($value->type == 'textarea')
                        <div class="relative flex flex-col mt-6">
                            ...
                            <p
                                class="character-count leading-5 text-[13px] font-medium text-color-89 text-right text-counterd absolute -bottom-6 right-0">
                                0/{{ preference('long_desc_length') }}
                            </p>
                        </div>
                    @endif
                @endforeach
        ...
        <!-- Provider -->
        <div
            class="custom-dropdown-arrow font-normal text-14 text-[#141414] dark:text-white {{ count($aiProviders) <= 1 ? 'hidden' : '' }}">
                        ...
                        <div class="gap-6 pt-3 grid grid-cols-2 ProviderOptions {{ $providerName . '_div' }}">
                            @foreach ($fields as $field)
                                @if ($field['type'] == 'dropdown')
                                    <div
                                        class="custom-dropdown-arrow font-normal text-14 text-[#141414] dark:text-white  {{ count($field['value']) <= 1 ? 'hidden' : '' }}">
