...
<form id="speech-to-text-form" class="bg-[#F6F3F2] dark:bg-[#3A3A39] font-Figtree h-full flex flex-col justify-between">
    <div
        class="bg-[#F6F3F2] dark:bg-[#3A3A39] xl:w-[401px] 5xl:w-[474px] sidebar-scrollbar xl:overflow-auto h-full pt-14">
        <div class=" px-5 py-[22px] !pb-0 sm:py-8 xl:p-6 pt-5">
            <p class="text-color-14 text-24 font-semibold font-RedHat dark:text-white wrap-anywhere">
                {{ __('Any voice, we’ll write it down!') }}</p>
            <p class="text-color-89 text-[13px] leading-5 font-medium font-Figtree mt-2 wrap-anywhere">
                {{ __(':x Works with any language and accent. Great for notes, messages, and more.', ['x' => preference('company_name')]) }}
            </p>
            @if ($minuteLeft && auth()->user()->id == $userId)
                <div class="bg-white dark:bg-[#474746] p-3 rounded-xl flex items-center justify-start mt-6 gap-2.5">
                    ...
                    <p
                        class="text-color-14 dark:text-white font-Figtree font-normal text-[14px] leading-[22px] wrap-anywhere">
                        {!! __('Credits Balance: :x mins left', [
                            'x' =>
                                "<span class='minute-credit-remaining font-semibold dark:text-[#32B9C5] text-[#32B9C5]'>" .
                                ($minuteLimit == -1 ? __('Unlimited') : ($minuteLeft < 0 ? 0 : formatDecimal($minuteLeft))) .
                                '</span>',
                        ]) !!}</p>
                </div>
            @endif
            <div class="flex flex-col mt-6">
                <div class="flex justify-between items-center mb-1.5">
                    <label class="font-normal text-14 text-color-2C dark:text-white require wrap-anywhere"
                        for="">{{ __('Upload Audio') }}</label>
                    <p class="text-[13px] leading-5 text-color-89 font-Figtree font-medium">
                        {{ __('Max size: :x MB', ['x' => preference('file_size')]) }}</p>
                </div>
            </div>
            <div
                class="file-drop-area border border-dashed border-color-89 rounded-xl bg-white dark:bg-color-33 dark:border-color-47 text-[13px] leading-[18px] font-normal font-Figtree text-color-14 wrap-anywhere text-center py-[37px] px-4 audio-speech relative">
                <div
                    class="file-msg justify-center items-center flex gap-2.5 text-color-14 dark:text-white line-clamp-single">
                ...
                <input class="file_input form-control" name="audio" id="file_input" required
                    oninvalid="this.setCustomValidity('{{ __('This field is required.') }}')" type="file">
                    ...
                    <select
                        ...
                        @foreach ($languages as $key => $language)
                            @if (in_array($language->short_name, $meta_languages))
                                <option value="{{ $language->short_name }}" {{ $key == 0 ? 'selected' : '' }}>
    ...
    <div class="bg-[#F6F3F2] dark:bg-[#3A3A39] py-6 w-full relative px-5 xl:px-6 h-max">
        <button
            class="magic-bg w-full rounded-xl text-16 bottom-0 z-[99999] text-white font-semibold py-3 flex justify-center items-center gap-3 transcribe-btn"
            id="text-generate">
            <span>
                {{ __('Transcribe') }}
            </span>
